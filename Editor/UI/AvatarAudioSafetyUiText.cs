...
namespace Sebanne.AvatarAudioSafetyGuard.Editor
{
    internal static class AvatarAudioSafetyUiText
    {
        ...
        public const string ReportTargetDescription = "このウィンドウは、選択した AvatarAudioSafetySettings に保存されている最新の Dry Run 結果を表示します。";
        public const string ReportWindowDescription = "Inspector の一覧と同じ見え方で、保存済みの診断結果を見返せます。";
        ...
        public const string ReportUsesStoredResultsDescription = "最新の Dry Run 結果は AvatarAudioSafetySettings に保持されます。ウィンドウを開き直しても、同じ設定を選べば再表示できます。";
        ...
        public const string ResultsPersistenceHint = "最新の走査結果は、この AvatarAudioSafetySettings に保持されます。";
        ...
        public const string ToolsSectionDescription = "Inspector からの走査は常に非破壊です。Build時に補正 では、Build 時だけ build clone に補正を適用します。";
        public const string DetectedAudioSourcesSectionDescription = "Dry Run で見つかった AudioSource 一覧です。ObjectField をクリックすると対象 GameObject を追えます。";
        ...
        public static string GetSummaryText(AvatarAudioScanSummary summary)
        {
            ...
            return string.Format(
                "走査数: {0} | 問題なし: {1} | 警告: {2} | Build時に補正予定: {3} | 報告のみ: {4} | 対象外: {5} | 手動確認: {6}",
                summary.scanned,
                summary.safe,
                summary.warnings,
                summary.wouldClamp,
